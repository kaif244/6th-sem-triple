...
function initMobileDrawerEngine() {
    ...
    drawer.addEventListener('touchstart', (e) => {
        // CHANGED: If user touches the drag handle bar directly, override scroll locks to drop panel immediately
        if (e.target.classList.contains('drawer-handle') || resultsContainer.scrollTop === 0 || e.target.closest('.search-section')) {
            touchStartY = e.touches[0].clientY;
        } else {
            touchStartY = null; // Lock normal card dragging if list is actively scrolling
        }
    }, { passive: true });
    ...
    drawer.addEventListener('touchmove', (e) => {
        ...
        if (deltaY < -40 && currentMobileState !== drawerState.EXPANDED) {
            ...
            // CHANGED: Handle bar drags will dismiss the panel back to middle/bottom seamlessly regardless of deep scroll position
            if (e.target.classList.contains('drawer-handle')) {
                if (currentMobileState === drawerState.EXPANDED) resetDrawerToMid();
